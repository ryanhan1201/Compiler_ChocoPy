...
#An imperial stone???
def imperialStone(x:int) -> int:
    if x < 5:
        return hailstone(x)
    else:
        return x + hailstone(imperialStone(x - 1) - 2)

#EVERYTHING EVERYWHERE ALL AT ONCE
def everythingAllAtOnceStone(x:int) -> int:
    if x % 2 == 0:
        return imperialStone(x)
    else:
        return imperialStone(x // 2) + everythingAllAtOnceStone(x // 2)
...
print(everythingAllAtOnceStone(input_int))